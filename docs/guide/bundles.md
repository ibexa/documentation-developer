...
This page shows how to import configuration from a bundle in two ways: [the manual way](#importing-configuration-manually) and [the implicit way](#importing-configuration-implicitly).
...
It relies on using the `imports` statement in your main `config/packages/ezplatform.yaml` file:
...
The `templates_rules.yaml` should be in the `Resources/config/` folder of the AcmeExampleBundle.
