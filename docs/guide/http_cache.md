...
!!! enterprise
    ...
    The easiest way to set environment variables is using the [Plaform.sh CLI](https://docs.platform.sh/gettingstarted/cli.html).
    Because this is an Enterprise project, you can set the values on the `production` branch specifically
    and they will be replicated to the production cluster.  Optionally you can set a different set of 
    credentials on the `staging` branch to test Fastly on the staging instance.
    
    Execute the following commands from your local git checkout:
    ...
    You may use the hostname already generated by Platform.sh (for instance `master-7rqtwti-qwertyqwerty.eu.platform.sh`).
    ...
    Next, add your hostname (for instance `foobar.com`) by clicking "Add domain".

    ### Configure Platform.sh routes (Platform Professional only)

    On Platform.sh Enterprise the route configuration is handled by Platform.sh. If however you are
    using Platform.sh Professional (essentially the development system but without a separate cluster for
    production) then you will need to define your own [route definition file](https://docs.platform.sh/configuration/routes.html), `.platform/routes.yaml`.

    The stock Platform.sh routes definition should be fine for the vast majority of cases.  The one
    exception is that you should disable Platform.sh's built-in HTTP cache as it is redundant with Fastly.
