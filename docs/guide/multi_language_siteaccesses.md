...
For more details on language versions, see [Internationalization](internationalization.md).

Create a new translation by performing the following steps:
...
Lastly, remove in `config/packages/ezplatform.yaml`, remove `site` from the `list` and `groups` sections.
...
Clear the cache again by running: `php bin/console cache:clear`.

Now you should be able to load your eZ Platform site using the `en` and `fr` SiteAccesses to display content in English and French.
