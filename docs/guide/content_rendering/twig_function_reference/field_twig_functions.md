...
`ez_render_field()` is the Twig function intended for rendering the Field on the front site.

You can get additional information about a Field using the following Twig functions:
...
- [`ez_field_is_empty()`](#ez_field_is_empty) returns information whether a Field of a Content item is empty.
...
The Field is rendered using the default template, but you can optionally pass a different template as parameter as well.
...
- `lang` - language to render the Field in (overrides the current language), must be a valid locale with xxx-YY format
...
To render the Field with default or custom templates, use [`ez_render_field()`](#ez-render-field) instead.
...
but it is recommended to use the dedicated [`ez_field_value()`](#ez-field-value) function for this.
...
For example, use `ez_field_is_empty()` to check if a Field is empty or filled before rendering it:
