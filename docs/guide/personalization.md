...
recommendationBundleRestRoutes:
    resource: '@EzSystemsRecommendationBundle/Resources/config/routing_rest.yaml'
...
In order to tell the recommender to use this user and password to request resources on the eZ Platform instance, you can configure this as follows (an example file is available in the bundle under `Resources/config/default_settings.yaml`):
...
Most operations are logged via the `ez_recommendation` [Monolog channel](http://symfony.com/doc/current/cookbook/logging/channels_handlers.html). To log everything about Recommendation to `dev.recommendation.log`, add the following to your `config.yaml`:
