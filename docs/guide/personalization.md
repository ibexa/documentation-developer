...
If the Content item's author or image are stored in a different Field,
...
{% if content is defined %}
    {{ recommendation_track_user(content.id) }}
{% endif %}
```
...
start the full export by executing the following command:
...
With this command, the bundle exporter collects all content related to the SiteAccesses of this customerID and stores it in files (1).
...
(based on [recommendations.html.twig](https://github.com/ezsystems/ezstudio-demo/blob/master/app/Resources/EzSystemsRecommendationBundle/views/recommendations.html.twig))
