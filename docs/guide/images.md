...
LiipImagineBundle only works on image blobs (no command line tool is needed). See the [bundle's documentation to learn more on that topic](http://symfony.com/doc/2.x/bundles/LiipImagineBundle/configuration.html).
...
Please refer to [post-processor documentation in LiipImagineBundle](http://symfony.com/doc/2.x/bundles/LiipImagineBundle/post-processors.html) for details.
...
If you were using ImageMagick, install [Imagick](http://php.net/imagick) or [Gmagick](http://php.net/gmagick) PHP extensions and activate the driver in `liip_imagine `([see LiipImagineBundle configuration documentation for more information](http://symfony.com/doc/2.x/bundles/LiipImagineBundle/configuration.html)):
...
You can use LiipImagine's `liip:image:cache:resolve` script to resolve the path to image variations generated from the original image, with one or more paths as arguments. See [LiipImagineBundle documentation](http://symfony.com/doc/2.x/bundles/LiipImagineBundle/commands.html#resolve-cache) for more information.
...
In addition to [filters exposed by LiipImagineBundle](http://symfony.com/doc/2.x/bundles/LiipImagineBundle/filters.html), the following are available:
...
LiipImagineBundle supports additional settings, it is possible to combine filters from the list above with [the ones provided in LiipImagineBundle](http://symfony.com/doc/2.x/bundles/LiipImagineBundle/filters.html) or custom ones.
...
Please refer to [LiipImagineBundle documentation on custom filters](http://symfony.com/doc/2.x/bundles/LiipImagineBundle/filters.html#custom-filters). [Imagine library documentation](http://imagine.readthedocs.org/en/latest/) may also be useful.
...
Placeholder generation can be configured for each [`binary_handler`](file_management.md#handling-binary-files) under the `ezpublish.image_placeholder` key:
...
If there is no configuration assigned to [`binary_handler`](file_management.md#handling-binary-files) name, the placeholder generation will be disabled.
