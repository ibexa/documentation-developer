...
!!! enterprise
    ...
    **Prerequisite**
    ...
    It may occur that you don't posses client certificate signed by public CA authority
    but you have self-signed CA certificate generated by `elasticsearch-certutil` tool 
    or different (for example for developing purposes). In this case you should use the following `ssl` configuration:
