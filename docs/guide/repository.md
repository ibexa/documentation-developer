...
When content is moved to a different Location, the item itself and all of its subtree will be assigned to the Section of the new Location. Please note that it works only for copy and move; updating parents' Location does not affect the subtree, meaning that Content Location cannot be updated this way.
