...
The number of web servers, Memcached/Redis/Valkey, Solr, Varnish, Database, and NFS servers, but also whether some servers play several of these roles (typically running Memcached/Redis/Valkey across the web server), is up to you and your performance needs.

The minimal requirements are:

- [Shared HTTP cache (using Varnish)](reverse_proxy.md#using-varnish-or-fastly)
- [Shared persistence cache](#shared-persistence-cache) and [sessions](#shared-sessions) (using Redis, Valkey, or Memcached)
...
An alternative solution is Memcached.
...
The main option out of the box in Symfony are the native PHP Memcached and the PHP Redis session handler (also compatible with Valkey).
