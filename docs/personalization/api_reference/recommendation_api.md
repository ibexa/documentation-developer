...
|`usetimeslot`| |If set to true, configured time-slots are active. As a result, recommendations are calculated for a specific time frames and they have priority over the recommendations from the main model in the hours for which time slots are configured. |true|
