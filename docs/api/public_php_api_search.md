...
To do this, you can use the `SearchService` described below or
[Repository filtering](public_php_api_filtering.md) if you don't need indexed data and want to 
benefit from no latency between a search engine and the Repository.
...
You can use the `SearchService` or [Repository filtering](public_php_api_filtering.md#filtering-in-a-controller) in a controller, as long as you provide the required parameters.
...
To see a similar but search engine independent example visit [Repository filtering](public_php_api_filtering.md#filtering-in-a-controller).
