...
To do this you can take advantage of eZ Platform's [HATEOAS-based](https://en.wikipedia.org/wiki/HATEOAS) REST API and extend it with custom Content Types for your own needs. In this procedure you will add comments count to `eZ\Publish\API\Repository\Values\Content\VersionInfo` responses.
...
To be able to use the overridden type you also need to implement new Compiler Pass. For more details, see [Symfony doc.](https://symfony.com/doc/3.4/service_container/compiler_passes.html)
