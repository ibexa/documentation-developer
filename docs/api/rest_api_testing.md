...
You can, however, try opening the root resource with it, using the session authentication: `http://example.com/api/ezp/v2/`.
...
```php
$resource = 'https://api.example.com/api/ezp/v2/content/objects/52';
$curl = curl_init($resource);
curl_setopt_array($curl, [
    CURLOPT_HTTPHEADER => ['Accept: application/vnd.ez.api.ContentInfo+json'],
...
```javascript
var resource = '/api/ezp/v2/content/objects/52',
