...
!!! note "Package Dependencies"
    ...
    Regardless of the type you set, always remember to specify package dependencies in Composer requirements so the dependencies are clear. For example if you reply on `ezsystems/landing-page-fieldtype-bundle` make sure to add that with the major versions you support of it.
