...
With swap enables you will be able to successfully run `php -d memory_limit=-1 app/console ezplatform:install --env prod ezplatform-clean`.

When a system runs out of RAM, you may see `Killed` when trying to clear the cache (e.g. `php app/console --env=prod cache:clear` from your project's root directory).
