...
=== "MCP Servers"
    ...
    - An [`allowed_hosts`](mcp_config.md#allowed-hosts) as been added restrincting usage to local host by default. You may have to set its value to allow more hosts.
    - The [built-in tool](mcp_config.md#built-in-tools) `list_content_translations` is now renamed `list_content_languages`.
