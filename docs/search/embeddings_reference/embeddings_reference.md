...
The `Ibexa\Contracts\Taxonomy\Search\Query\Value\TaxonomyEmbedding` class allows embedding queries to target taxonomy data.
...
- `Ibexa\Contracts\Core\Repository\Values\Content\EmbeddingQuery` represents a semantic similarity search request.
...
- `Ibexa\Contracts\Core\Repository\Values\Content\Query\Embedding` represents the vector input used
...
- `Ibexa\Contracts\Core\Search\Embedding\EmbeddingProviderRegistryInterface` lists available embedding providers or gets one by its identifier

- `Ibexa\Contracts\Core\Search\Embedding\EmbeddingProviderResolverInterface` determines the embedding provider to be used for generating embeddings based on the system configuration, or a demand passed through the `resolveByModelIdentifier` method
...
- EmbeddingConfigurationInterface allows access to the embedding model configuration in the system (for example, list of available models, default model name, default provider, field suffix, and so on)
...
- `Ibexa\Contracts\Core\Repository\Values\Content\QueryValidatorInterface` validates embedding query structure before execution
