...
```bash
./tools/clone-repositories.sh [DEVDOC_60] [DEVDOC_50] [DEVDOC_46] [USERDOC_SAAS] [USERDOC_60] [USERDOC_50] [USERDOC_46] [CONNECT]
```
...
| `USERDOC_SAAS` | `ibexa/documentation-user`      | `saas`  |
...
documentation, so `doc.ibexa.co/en/saas/` links are remapped to the local `site/`
build instead.

Example — checking link for release PRs:

```bash
./tools/clone-repositories.sh 6.0 release-5.0.10 release-4.6.70 saas 6.0 4.6 4.6 main
```
