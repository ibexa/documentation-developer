...
Data export schedule in Ibexa CDP.

Editions: Experience

## Configuration key

Configuration in Ibexa CDP allows you to automate the process of exporting content, users, and products. An `ibexa_cdp.data_export` [configuration key](../../administration/configuration/configuration/index.md#configuration-files) looks as below:
...
- `interval` - sets the frequency of the command invoke, for example, `'*/30 * * * *'` means "every 30 minutes", `'0 */12 * * *'` means "every 12th hour". It uses a standard `crontab` format, see [examples](https://crontab.guru/examples.html).
...
For more information, see [Schedule tasks](../../getting_started/install_ibexa_dxp/index.md#schedule-tasks).
