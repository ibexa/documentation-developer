...
Customize the templates used to render products embedded in RichText fields.

When a product is [embedded in a RichText field](../products/index.md#embed-products-in-content), it is rendered by using a Twig template. You can override the default templates to customize the appearance of embedded products.
...
| ------------- | ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `product`     | `product`, `product_inline`                                                                | A [`Ibexa\Contracts\ProductCatalog\Values\ProductInterface`](../../../../../ibexa/product-catalog/src/contracts/Values/ProductInterface.php) object. |
| `productCode` | `product_denied`, `product_inline_denied`, `product_not_found`, `product_inline_not_found` | The product code string, used to identify the product that could not be loaded.                                                                               |
| `embedParams` | All block types                                                                            | Optional parameters set by the online editor, for example `align` or `class` properties                                                                       |
...
To override a template, create a file with the same name in your [theme directory](../../templating/design_engine/design_engine/index.md). For example, to override the block embed template for the `standard` theme, create a file in `templates/themes/standard/product_catalog/richtext/embed/product.html.twig`
...
In addition to overriding the templates with the [design engine](../../templating/design_engine/design_engine/index.md), you can explicitly set the template path for any embed type in your [SiteAccess configuration](../../multisite/multisite_configuration/index.md):
