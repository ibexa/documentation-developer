...
>
> You can set [different version constraints](https://getcomposer.org/doc/articles/versions.md), for example, specific tag (`5.0.10`), version range (`~5.0.1`), or stability (`^5.0@rc`):
>
> ```bash
> composer create-project ibexa/experience-skeleton:5.0.10 .
> ```
...
In `DATABASE_VERSION`, you can also configure the database server version. For the expected version format, see [Automatic platform version detection](https://www.doctrine-project.org/projects/doctrine-dbal/en/3.10/reference/configuration.html#automatic-platform-version-detection).
