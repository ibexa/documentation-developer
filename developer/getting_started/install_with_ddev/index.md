...
Install Ibexa DXP with Docker and DDEV to use it for development.

This guide provides a step-by-step walkthrough of installing Ibexa DXP by using [DDEV](https://ddev.com/). DDEV is an open-source tool that simplifies the process of setting up local PHP development environments.
...
You can also use other versions of MariaDB, MySQL or PostgreSQL. See [DDEV database types documentation](https://docs.ddev.com/en/stable/users/extend/database-types/) for available version ranges.
...
> **Note: Note**
>
> Those commands set a `DATABASE_URL` environment variable inside the container which overrides [the variable from `.env`](../install_ibexa_dxp/index.md#change-installation-parameters).
>
...
You can configure [Symfony Mailer](https://symfony.com/doc/7.4/mailer.html) to use the [integrated mail catcher Mailpit](https://docs.ddev.com/en/stable/users/usage/developer-tools/#email-capture-and-review-mailpit):
...
You can [schedule tasks](../install_ibexa_dxp/index.md#schedule-tasks) using [DDEV Cron add-on](https://addons.ddev.com/addons/ddev/ddev-cron).
...
For more schedulable tasks and ways to schedule them, see [Additional scheduled tasks and advanced usage](../install_ibexa_dxp/index.md#additional-scheduled-tasks-and-advanced-usage).
...
You can launch [Ibexa Messenger](../../infrastructure_and_maintenance/background_tasks/index.md) on DDEV project start. Create or edit a DDEV config file, for example `.ddev/config.hooks.yaml`, and add the following [hook](https://docs.ddev.com/en/stable/users/configuration/hooks/):
...
If you're using macOS or Windows, you might want to enable [Mutagen](https://docs.ddev.com/en/stable/users/install/performance/#mutagen) to improve performance. You can do this by running the following command:
...
By default, DDEV uses ports 80 and 443. You can [set different ports](https://docs.ddev.com/en/stable/users/usage/troubleshooting/#method-2-fix-port-conflicts-by-configuring-your-project-to-use-different-ports) with a command like the following:
...
Next, you need to [set up authentication tokens](../install_ibexa_dxp/index.md#set-up-authentication-tokens) by modifying the Composer configuration. You must run the following command **after** executing `ddev start`, because the command runs inside the container. Replace `<installation-key>` and `<token-password>` with your actual installation key and token password.

```bash
ddev composer config --global http-basic.updates.ibexa.co <installation-key> <token-password>
```

This authentication doesn't persist if the project is restarted (by `ddev restart` or `ddev composer create-project`). You can back up the authentication file (`auth.json`) by using the following command:
...
Once DDEV is running, use Composer to create a new Ibexa DXP project. Remember to replace `<edition>` and `<version>` with your desired edition and version.
...
The installer will prompt you for a new password for the `admin` user. The password must meet the following rules:
...
Instead of using environment variables inside the container, a [`.env.local`](https://symfony.com/doc/7.4/configuration.html#overriding-environment-values-via-env-local) file can be added to the project.
...
- Modify step [5. Create Ibexa DXP project](#5-create-project) to insert the database setting:
...
5. Populate the contents, which could mean:
   - getting a clean database with `ddev php bin/console ibexa:install` and adding some data with [Ibexa data migration](../../content_management/data_migration/importing_data/index.md), or
...
The following examples run an already [version-controlled project](../install_ibexa_dxp/index.md#add-project-to-version-control) and have the right content structure (but no content):
...
Notice that the example adds the whole `.ddev/` directory to `.gitignore`, but you can also version parts of it. Some DDEV configs can be shared among developers. For example, a common `.ddev/config.yaml` can be committed for everyone and [locally extended or overridden](https://docs.ddev.com/en/stable/users/extend/customization-extendibility/#extending-configyaml-with-custom-configyaml-files).
...
- In [5. Create Ibexa DXP project](#5-create-project), use only `ddev composer install` instead of `ddev composer create-project`.
- Populate the database with [Ibexa data migration](../../content_management/data_migration/importing_data/index.md) or [`ddev import-db`](https://docs.ddev.com/en/stable/users/usage/commands/#import-db).

### Hostnames and domains

If the local project needs to answer to real production domains (for example, to use the existing [hostname to SiteAccess](../../multisite/siteaccess/siteaccess_matching/index.md#maphost) or [hostname element to SiteAccess](../../multisite/siteaccess/siteaccess_matching/index.md#hostelement) mappings), you can use [additional hostnames](https://docs.ddev.com/en/stable/users/extend/additional-hostnames/).
...
- See [Clustering with DDEV](../../infrastructure_and_maintenance/clustering/clustering_with_ddev/index.md) to add Elasticsearch, Solr, or Redis to your DDEV installation.
- See [DDEV and Ibexa Cloud](../../ibexa_cloud/ddev_and_ibexa_cloud/index.md) to locally run an Ibexa DXP project by using DDEV.
