...
To add custom icons for existing content types or custom content types in Ibexa DXP, use the following configuration under the `ibexa.system.<scope>.content_type` [configuration key](../../../configuration/configuration/index.md#configuration-files):
...
Use the [scope](../../../../multisite/multisite_configuration/index.md#scope) if you want different icons for different SiteAccesses.

To see more Admin UI's `ids-assets` icons, see [the icon reference](../../../../templating/twig_function_reference/icon_twig_functions/index.md#icons-reference).

### Access icons in Twig templates

#### Content type icons

Content type icons are accessible in Twig templates via the [`ibexa_content_type_icon()` function](../../../../templating/twig_function_reference/icon_twig_functions/index.md#ibexa_content_type_icon). It requires content type identifier as an argument. The function returns the path to a content type icon.
...
User interface icons are accessible with [`ibexa_icon_path()` function](../../../../templating/twig_function_reference/icon_twig_functions/index.md#ibexa_icon_path). The function returns a path from an icon identifier and an [icon set](#icon-sets) identifier arguments.
...
You can retrieve the icon URL with the `getContentTypeIconUrl` helper function that is set on the global `ibexa.helpers.contentType` object. It takes content type identifier as an argument and returns one of the following items:
...
The following example from the `alert.js` file shows configuration for icons in the [alert](../reusable_components/index.md#alerts) component:
...
The icon sets are used by [`ibexa_icon_path()` Twig function](../../../../templating/twig_function_reference/icon_twig_functions/index.md#ibexa_icon_path).
...
The built-in default icon set is `ids-assets` (corresponding to `/bundles/ibexaadminuiassets/vendors/ids-assets/dist/img/all-icons.svg`). To see the icons available in this set, see [the icon reference](../../../../templating/twig_function_reference/icon_twig_functions/index.md#icons-reference).
