...
> **Note: Note**
>
> Maximum size is 10MB. We recommend setting the `upload_max_filesize` key in the `php.ini` configuration file to a value equal to or higher than that. It prevents validation errors while editing content types.

## Using an Image field

To read more about handling images and image variations, see the [Images documentation](../../../images/images/index.md).
...
The template called by the [`ibexa_render_field()` Twig function](../../../../templating/twig_function_reference/field_twig_functions/index.md#ibexa_render_field) while rendering a Image field accepts the following parameters:
...
Children of the `fieldValue` node list the general properties of the field's original image (for example, `fileSize`, `fileName`, or `inputUri`), and its variations. For each variation, a URI is provided. Requested through REST, this resource generates the variation if it doesn't exist yet, and list the variation details:
...
The variation service, `ibexa.field_type.ibexa_image.variation_service`, can be used to generate/get variations for a field. It expects a VersionInfo, the Image field, and the variation name as a string (`large`, `medium`, and more.):
