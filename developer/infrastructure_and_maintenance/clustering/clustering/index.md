...
- [Shared HTTP cache (using Varnish)](../../cache/http_cache/reverse_proxy/index.md#using-varnish-or-fastly)
...
For more information on requirements, see [Requirements page](../../../getting_started/requirements/index.md).

It's also recommended to use:

- [Solr](../../../search/search_engines/solr_search_engine/solr_overview/index.md) or [Elasticsearch](../../../search/search_engines/elasticsearch/elasticsearch_overview/index.md) for better search and performance
...
- more recent versions of PHP and MySQL/MariaDB within [what is supported](../../../getting_started/requirements/index.md) for your Ibexa DXP version to get more performance out of each server. Numbers might vary so make sure to test this when upgrading.

### Shared persistence cache

Redis and Valkey are the recommended cache solutions for clustering.

See [persistence cache documentation](../../cache/persistence_cache/index.md#persistence-cache-configuration) on information on how to configure them.

### Shared sessions

For a [cluster](index.md) setup you need to configure sessions to use a back end that is shared between web servers. The main option out of the box in Symfony is the PHP Redis session handler (also compatible with Valkey). Alternatively, there is Symfony session handler for PDO (database).
...
Ibexa DXP supports multi-server setups by means of [custom IO handlers](../../../content_management/file_management/file_management/index.md#dfs-cluster-handler). They make sure that files are correctly synchronized among the multiple clients using the data.
...
You can read more about that on [Binary files URL handling](../../../content_management/file_management/file_url_handling/index.md#file-url-handling).
...
While the command is running, the files should not be modified. To avoid surprises you should create a [backup](../../backup/index.md) and/or execute a dry run before doing the actual update, using the `--dry-run` switch.
