...
> **Caution: Caution**
>
> Don't use this procedure in production. A staging environment for validation before production should exactly replicate the production environment. This is meant for development environment only.

This guide follows [Install with DDEV](../../../getting_started/install_with_ddev/index.md) and helps to extend the previous installation to locally replicate a production [cluster](../clustering/index.md).
...
To run an Ibexa Cloud project locally, you may refer to [DDEV and Ibexa Cloud](../../../ibexa_cloud/ddev_and_ibexa_cloud/index.md) instead.

## Install reverse proxy

A reverse proxy can be added to the cluster to enable [HTTP caching](../../cache/http_cache/http_cache/index.md).
...
If you're using [Apache as web server](../../../getting_started/install_with_ddev/index.md#switch-to-apache-and-its-virtual-host), you must set `varnish` as a trusted proxy in `.ddev/apache/apache-site.conf` before restarting DDEV:
...
The Varnish server acts as the application’s primary entry point. If you run `ddev describe`, you can see that Varnish is now the one responding to DDEV domain `.ddev.site` while the web server still replies to `127.0.0.1` with its own ports.
...
To explore more the communication between the web server and Varnish, you can find other examples of requests done directly to the web server while impersonating Varnish in [Fetching user context hash](../../cache/http_cache/content_aware_cache/index.md#fetching-user-context-hash) and [Fetching HTML response](../../cache/http_cache/content_aware_cache/index.md#fetching-html-response).

You can use `ddev varnishlog` command to monitor Varnish logs in real time. Due to how parameters are passed to the container, you may have to wrap some parameters in quotes twice, for example, the purge request monitoring:
...
For Fastly (as for [Ibexa Connect](https://doc.ibexa.co/projects/connect/en/latest/)), the instance must be visible from Internet.
...
Be careful when making a local development instance visible from the internet. For example:
...
- don't store it on a Fastly or Ibexa Connect accounts used by external people

See [Configure and customize Fastly](../../cache/http_cache/fastly/index.md) for the Fastly side.

## Install search engine

A [search engine](../../../search/search_engines/search_engines/index.md) can be added to the cluster.

### Elasticsearch

The installation of Elasticsearch within a DDEV stack is an adaptation of the [on-premise installation](../../../search/search_engines/elasticsearch/install_elasticsearch/index.md) procedure using the [`ddev/ddev-elasticsearch` add-on](https://addons.ddev.com/addons/ddev/ddev-elasticsearch).
...
The installation of Solr within a DDEV stack is an adaptation of the [on-premise installation](../../../search/search_engines/solr_search_engine/install_solr/index.md) procedure using the [`ddev/ddev-solr` add-on](https://addons.ddev.com/addons/ddev/ddev-solr).

For example, the following sequence of commands:

1. Adds the Solr container
2. Sets Solr as the search engine
3. Start the DDEV cluster to creates core config by combining default files and those provided by Ibexa DXP
4. Restarts the DDEV cluster and clears application cache
...
You can add a [persistence cache pool](../../cache/persistence_cache/index.md#persistence-cache-configuration) and a [session handler](../../sessions/index.md#session-handlers) to the cluster.
...
4. Changes `maxmemory-policy` from default `allkeys-lfu` to a [value accepted by the `RedisTagAwareAdapter`](https://github.com/symfony/cache/blob/5.4/Adapter/RedisTagAwareAdapter.php#L95).
