...
To authenticate REST API communication you can use session (default), JWT, basic, OAuth and client certificate (SSL) authentication.

This page refers to [REST API reference](https://doc.ibexa.co/en/5.0/api/rest_api/rest_api_reference/rest_api_reference.html), where you can find detailed information about REST API resources and endpoints.
...
- [Cross-origin requests](../rest_api_usage/rest_responses/index.md#cross-origin)
- [`access_control`](https://symfony.com/doc/7.4/security/access_control.html)

> **Caution: SiteAccess login**
>
> The anonymous user is used to perform authentification requests. Therefore, the "Anonymous" role must have `user/login` permission on the SiteAccess that matches the REST domain or is passed through the [`X-Siteaccess` header](../rest_api_usage/rest_requests/index.md#siteaccess).

## Session-based authentication

This authentication method requires a session cookie to be sent with each request.

If you use this authentication method with a web browser, this session cookie is automatically available as soon as your visitor logs in. Add it as a cookie to your REST requests to authenticate the user.
...
You can create a session for a visitor even if they're not logged in by sending the **`POST`** request to `/user/sessions`. To log out, use the **`DELETE`** request on the same resource.
...
It can be important to keep the CSRF token (`csrfToken`) for the duration of the session, because you must send this token in every request that uses [unsafe HTTP methods](../rest_api_usage/rest_requests/index.md#request-method) (others than the safe GET or HEAD or OPTIONS) when a session has been established. It should be sent with an `X-CSRF-Token` header.
...
The purpose of CSRF protection is to prevent users from accidentally running harmful operations by being tricked into executing an HTTP(S) request against a web applications they're logged into. In browsers this action is blocked by lack of CSRF token.
...
See [JWT authentication](../../../infrastructure_and_maintenance/security/development_security/index.md#jwt-authentication) for configuration instructions.
...
To obtain a JWT token with REST, you can use the live API documentation that is available on your development installation. This documentation is only accessible when `kernel.debug` is set to `true`, similarly to a development environment.
...
![REST API live documentation with a JWTInput payload](https://doc.ibexa.co/en/5.0/api/rest_api/img/jwt-rest-doc-request.png "REST doc JWT token request") ![REST API live documentation with a JWTInput payload](https://doc.ibexa.co/en/5.0/api/rest_api/img/jwt-rest-doc-response.png "REST doc JWT token response")
...
>
> If you enable basic authentication for `pattern: ^/api/ibexa/v2` to use it in your front office across both production and development environments, your development environment's back office cannot work correctly. This back office tries to access REST through the same URL as the front office. Even when logged in back office and using the [X-SiteAccess header](../rest_api_usage/rest_requests/index.md#siteaccess), the firewall blocks access to REST as you're not logged through basic authentification. Therefore, some back office features don't work.
>
...
Basic authentication requires the username and password to be sent *(username:password)*, base64 encoded, with each request. For details, see [RFC 2617](https://datatracker.ietf.org/doc/html/rfc2617).

Most HTTP client libraries and REST libraries support this method. [Creating content with binary attachments](../rest_api_usage/rest_requests/index.md#creating-content-with-binary-attachments) is an example of using basic authentication with [cURL](https://www.php.net/manual/en/book.curl.php) and its `CURLOPT_USERPWD`.
