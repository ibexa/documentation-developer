...
The REST API covers objects in the Ibexa DXP Repository with regular and custom HTTP methods, such as GET or PUBLISH, and HTTP headers.

The REST API in Ibexa DXP allows you to interact with an Ibexa DXP installation by using the HTTP protocol, following a [REST](https://en.wikipedia.org/wiki/Representational_state_transfer) interaction model.

Each resource (URI) interacts with a part of the system (like content, users or search). Every interaction with the repository than you can do from back office or by using the [Public PHP API](../../../php_api/php_api/index.md) can also be done with the REST API.
...
- [YAML format](https://doc.ibexa.co/en/5.0/api/rest_api/rest_api_reference/openapi.yaml)
- [JSON format](https://doc.ibexa.co/en/5.0/api/rest_api/rest_api_reference/openapi.json)
...
> **Note: Note**
>
> In [Symfony's `dev` environment](../../../../infrastructure_and_maintenance/environments/index.md), you can access a REST API reference generated for your project by visiting the `/api/ibexa/v2/doc` route in the browser.
...
[REST reference](https://doc.ibexa.co/en/5.0/api/rest_api/rest_api_reference/rest_api_reference.html), for the sake of readability, uses no prefixes in the URIs. In practice, the `/api/ibexa/v2` prefixes all REST hrefs.

This prefix immediately follows the domain, and you can't use the [`URIElement` SiteAccess matcher](../../../../multisite/siteaccess/siteaccess_matching/index.md#urielement). If you need to the select a SiteAccess, see the [`X-Siteaccess` HTTP header](../rest_requests/index.md#siteaccess).
