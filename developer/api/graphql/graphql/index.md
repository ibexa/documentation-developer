...
GraphQL enables making concise, readable requests to Ibexa DXP APIs.

[GraphQL](https://graphql.org/) is a query language for the API. The GraphQL implementation for Ibexa DXP is located in [`ibexa/graphql`](https://github.com/ibexa/graphql).
...
YAML files with the schema are located in `config/graphql/types/ibexa`. They contain information about the domain objects and the fields you can [query](../graphql_queries/index.md) and [operate on](../graphql_operations/index.md).
...
The repository schema, accessed through `_repository`, exposes the Ibexa DXP repository in a manner similar to the [Public PHP API](../../php_api/php_api/index.md).

The `_repository` field also enables you to query, for example, object states configured for the repository.

### Custom schemas

You can also use your own [custom schema](../graphql_customization/index.md#custom-schema).
...
If you have [JWT authentication](../../../infrastructure_and_maintenance/security/development_security/index.md#jwt-authentication) enabled, you can use the following query to get your authentication token:
...
To obtain a JWT token, you can use the GraphiQL interface on your development installation. GraphiQL is only accessible when `kernel.debug` is set to `true`, similarly to a development environment.
...
![GraphiQL with a JWT token request and its response](https://doc.ibexa.co/en/5.0/ai/mcp/img/jwt-graphiql.png "GraphiQL JWT token request and response")
...
The [GraphiQL interactive client](https://github.com/graphql/graphiql) is included in the installation. Access it through `<yourdomain>/graphiql`.
