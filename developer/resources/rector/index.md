...
Use Ibexa  Rector, an optional package based on Rector, to remove PHP and JavaScript code deprecations.

Ibexa Rector is an optional package based on [Rector](https://getrector.com/) that comes with additional rule sets for working with Ibexa DXP code. Use it to get rid of PHP and JavaScript code deprecations and prepare your project for the next major release.
...
- selecting the Ibexa rule set that matches your current version, for example [`IbexaSetList::IBEXA_50`](../../../../../ibexa/rector/src/contracts/Sets/IbexaSetList.php)
...
Ibexa Rector also comes with transform module to help you maintain your JavaScript code.
...
Array of objects with input and output directories for transformed files, relative to your project or bundle root. Directory structure is not modified during the transformation.

#### `prettierConfigPath`

[Prettier](https://prettier.io/) is run at the end of the transformation. You can provide the path to your own configuration file, otherwise [the default file](https://github.com/ibexa/eslint-config-ibexa/blob/main/prettier.mjs) is used.

#### `plugins`

Use it to modify enabled plugins. To learn more about plugins, see [the list of plugins](#list-of-plugins).
...
The plugin configuration is an object with plugin names as keys, for example `ibexa-rename-string-values`. Inside a single plugin configuration, the property names are values that should be replaced. They can be specified explicitly (`ezform-error`) or by using regexp (`(^|\\s)\\.ez-`).
...
This plugin allows to change translation IDs. Extract translations after running this transformation.
...
This plugin changes values in translations. Extract translations after running this transformation.
...
If the `selectors-only` property is set to `true`, this plugin changes only strings inside HTML tags. Set to `false` or remove property to change text values as well.
