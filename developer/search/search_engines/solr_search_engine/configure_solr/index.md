...
Configure Solr search engine to use it with Ibexa DXP.
...
>
> Boosting of fields or documents affects the score (relevance) of your search result hits when using Search API for any Criteria you specify on `$query->query`, or in REST by using `Query` element. When you don't specify anything to sort on, the result is sorted by this relevance. Anything set on `$query->filter`, or in REST by using `Filter` element, *doesn't* affect scoring and only works as a pure filter for the result. Thus make sure to place Criteria you want to affect scoring on `query`.
...
>
> The following example presents boosting configuration for Folder's `name` and `description` fields. First, in `ibexa_solr.yaml` configure [custom fulltext fields](https://github.com/Novactive/NovaeZSolrSearchExtraBundle/blob/master/doc/custom_fields.md).
>
> ```yaml
...
First you need to change the core configuration in `solrconfig.xml` (for example `*/opt/solr/server/ibexa/collection1/conf/solrconfig.xml`). You can copy and paste the code below before any other `requestHandler` section.
...
![Solr Slave](https://doc.ibexa.co/en/5.0/search/img/solr.png)
...
To learn how you can create document field mappers, custom Search Criteria, custom Sort Clauses and Aggregations, see [Search extensibility](../../../extensibility/create_custom_search_criterion/index.md).
