...
### Unresolved references in the REST OpenAPI schema

`redocly build-docs` fails with `Error: Invalid reference token: <name>` when the
dumped schema contains a `$ref` to a component it does not define. The message
carries no file, path or line, so the build lints the schema first, with
`tools/api_refs/redocly-lint.yaml`, to report the endpoint and line behind it.
The lint is deliberately non-fatal: `build-docs` remains the gate.

The schema is generated by API Platform from the `#[Get]`/`#[Post]`/… attributes
on the REST controllers, so such a reference is a product-side issue, not a doc
one. Report it against the package owning the endpoint the lint names.

When the build fails, the schema it was built from is kept as the
`openapi-schema` artifact of the GitHub Actions run.
